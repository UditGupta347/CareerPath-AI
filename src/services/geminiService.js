...
export async function extractSkillsWithAI(text, context = 'resume') {
  ...
  try {
    ...
    const prompt = `You are an expert technical recruiter and skill analyst. Extract ONLY the technical and soft skills that are EXPLICITLY mentioned in the following ${context}.

${context === 'github' ? 'GitHub Profile Data:' : 'Text:'}
${text}

CRITICAL INSTRUCTIONS FOR ACCURATE SKILL EXTRACTION:

1. ONLY extract skills that are EXPLICITLY MENTIONED in the text:
   - Direct mentions: "JavaScript", "React", "Python"
   - Clear abbreviations in context: "built with JS" → "JavaScript"
   - Technologies explicitly named in project descriptions

2. DO NOT extract:
   - Single letters unless they're clearly programming languages (e.g., "R programming language" → OK, random "r" → NO)
   - Implied or assumed skills not mentioned in the text
   - Skills from generic words (e.g., "rest" as in "rest of the project" is NOT "REST API")
   - Technologies you think they MIGHT know but aren't mentioned

3. For abbreviations, extract the FULL FORM ONLY:
   - "JS" or "JavaScript" → extract only "JavaScript"
   - "ML" or "Machine Learning" → extract only "Machine Learning"
   - "TS" or "TypeScript" → extract only "TypeScript"
   - DO NOT extract both the abbreviation AND full form

4. Programming language "R":
   - ONLY extract if explicitly mentioned as "R programming", "R language", "R statistical"
   - DO NOT extract from random words containing the letter 'r'

5. Use PROPER capitalization and canonical names:
...
6. Validation rules:
   - Each skill must be a real, recognized technology or framework
   - No single letters unless they're well-known languages (C, R when explicitly stated)
   - No duplicates (don't list "React" and "ReactJS" separately)
   - No generic terms (avoid "programming", "development" unless it's "Web Development" as a category)

Return a JSON object with this structure (NO markdown, just JSON):
{
  "languages": ["Only explicitly mentioned programming languages - use full names"],
  "frameworks": ["Only explicitly mentioned frameworks and libraries - use canonical names"],
  "tools": ["Only explicitly mentioned development tools, Git, Docker, etc."],
  "databases": ["Only explicitly mentioned database technologies"],
  "cloud": ["Only explicitly mentioned cloud platforms"],
  "soft": ["Only explicitly mentioned soft skills"],
  "other": ["Only explicitly mentioned other technical skills like REST API, GraphQL, etc."]
}

Examples:
✅ CORRECT:
- Text: "Experience with JS and React" → languages: ["JavaScript"], frameworks: ["React"]
- Text: "Built with Python, Django" → languages: ["Python"], frameworks: ["Django"]
- Text: "R programming for statistics" → languages: ["R"]

❌ INCORRECT:
- Text: "Rest of the project uses Node" → other: ["REST API"] ❌ NO! "rest" here means "remainder"
- Text: "experienced developer" → languages: ["R"] ❌ NO! Just because there's an 'r' doesn't mean R language
- Text: "JavaScript developer" → languages: ["JavaScript", "JS"] ❌ NO! Only list "JavaScript" once

BE PRECISE - only extract what is CLEARLY and EXPLICITLY stated!`;
...
function extractSkillsFallback(text) {
  ...
  const skillPatterns = {
    languages: [
      ...
      // Removed standalone 'r', 'matlab', 'sql' as they cause too many false positives
      // Only match R when explicitly stated as 'r programming' or 'r language'
    ],
    frameworks: [
  ...
  // Extract skills with word boundary checking to avoid false positives
  for (const [category, patterns] of Object.entries(skillPatterns)) {
    ...
    patterns.forEach(pattern => {
      ...
      if (regex.test(text)) {
        ...
        if (specialCases[pattern.toLowerCase()]) {
          capitalizedSkill = specialCases[pattern.toLowerCase()];
        } else {
          // Default capitalization: capitalize first letter of each word
          capitalizedSkill = pattern.split(' ').map(word => 
  ...
  // Special handling for R programming language - only add if explicitly mentioned
  if (/\bR\s+(programming|language|statistical|studio|package)/i.test(text)) {
    extracted.languages.push('R');
  }
