...
export async function extractSkillsFromText(text, source = 'resume') {
  try {
    ...
    // Import deduplicateSkills utility
    const { deduplicateSkills } = await import('../utils/skillNormalizer');
    
    // Deduplicate each category
    const deduplicatedSkills = {
      languages: deduplicateSkills(extractedSkills.languages || []),
      frameworks: deduplicateSkills(extractedSkills.frameworks || []),
      tools: deduplicateSkills(extractedSkills.tools || []),
      databases: deduplicateSkills(extractedSkills.databases || []),
      cloud: deduplicateSkills(extractedSkills.cloud || []),
      soft: deduplicateSkills(extractedSkills.soft || []),
      other: deduplicateSkills(extractedSkills.other || [])
    };
    
    console.log('📝 Extracted and deduplicated skills:', deduplicatedSkills);
...
export async function compareWithRole(userSkills, targetRole) {
  try {
    ...
    const { findSkillMatches, deduplicateSkills } = await import('../utils/skillNormalizer');
    ...
    // Deduplicate user skills to avoid false positives
    const deduplicatedUserSkills = deduplicateSkills(userSkillsList);
    
    console.log('🔍 User skills before deduplication:', userSkillsList);
    console.log('🔍 User skills after deduplication:', deduplicatedUserSkills);
    console.log('🎯 Target role:', targetRole);
    console.log('📋 Required skills:', roleDetails.required);
    
    
    const requiredResult = findSkillMatches(deduplicatedUserSkills, roleDetails.required);
    console.log('✅ Required matches:', requiredResult.matches);
    console.log('❌ Required gaps:', requiredResult.gaps);
    
    
    const preferredResult = findSkillMatches(deduplicatedUserSkills, roleDetails.preferred);
    console.log('✅ Preferred matches:', preferredResult.matches);
    console.log('❌ Preferred gaps:', preferredResult.gaps);
    
    
    const toolsResult = findSkillMatches(deduplicatedUserSkills, roleDetails.tools);
    ...
    const aiRecommendations = await analyzeSkillGap(deduplicatedUserSkills, requiredResult.gaps);
