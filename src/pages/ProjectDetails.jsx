...
export default function ProjectDetails() {
  ...
  const { data: savedProject } = useQuery({
    ...
    queryFn: async () => {
      const saved = await api.entities.SavedProject.filter({
  ...
  const loadRoadmap = async () => {
    ...
    try {
      ...
      const getOrGenerateRoadmap = useProjectStore.getState().getOrGenerateRoadmap;

      const generatedRoadmap = await getOrGenerateRoadmap(project);
      setRoadmap(generatedRoadmap);
    } catch (error) {
      console.error('Error loading/generating roadmap:', error);
      setRoadmapError('Failed to load roadmap. Please check your API configuration.');
    } finally {
  ...
  const handleRegenerateRoadmap = async () => {
    ...
    const { useProjectStore } = await import('@/store/projectStore');
    const clearGeneratedRoadmap = useProjectStore.getState().clearGeneratedRoadmap;
    clearGeneratedRoadmap(project.id);
  ...
  const saveProjectMutation = useMutation({
    mutationFn: async () => {
      ...
      if (savedProject) {
        await api.entities.SavedProject.delete(savedProject.id);
      } else {
  ...
  const updateMilestoneMutation = useMutation({
    mutationFn: async (milestoneId) => {
      ...
      const totalTasks = project?.roadmap?.phases.reduce((acc, p) => acc + (p.tasks?.length || 0), 0);
      const progressPercent = totalTasks > 0 ? Math.round((newCompleted.length / totalTasks) * 100) : 0;

      if (savedProject) {
        
        await api.entities.SavedProject.update(savedProject.id, {
          ...
          <Tabs defaultValue="roadmap" className="w-full">
            ...
            <TabsContent value="roadmap" className="mt-0">
              <RoadmapView
                ...
                isGenerated={!project?.roadmap?.phases}
              />
            </TabsContent>
