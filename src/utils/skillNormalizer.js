...
export const SKILL_SYNONYMS = {
  ...
  'mysql': ['mysql', 'my sql'],
  'sql': ['sql', 'structured query language', 'sql database', 'mysql', 'postgresql', 'postgres', 'sqlite', 'mariadb', 'oracle sql', 'ms sql'],
  ...
  'sass': ['sass', 'scss', 'syntactically awesome style sheets'],
  'tailwind': ['tailwind', 'tailwindcss', 'tailwind css'],
  'bootstrap': ['bootstrap', 'bootstrap css'],
  
  // Design & Architecture
  'responsive design': ['responsive design', 'responsive', 'mobile responsive', 'responsive web design'],
  'database design': ['database design', 'db design', 'schema design', 'data modeling'],
  ...
  'api design': ['api design', 'api architecture', 'rest design'],
...
export function fuzzySkillMatch(userSkill, requiredSkill) {
  ...
  const userNormalized = normalizeSkill(userSkill).toLowerCase();
  const requiredNormalized = normalizeSkill(requiredSkill).toLowerCase();
  ...
  // Check if either skill is in the other's synonym list
  const userSynonyms = SKILL_SYNONYMS[userNormalized] || [];
  const requiredSynonyms = SKILL_SYNONYMS[requiredNormalized] || [];
  
  // Check if required skill matches any user skill synonym
  if (userSynonyms.some(syn => syn.toLowerCase() === required)) {
    return true;
  }
  
  // Check if user skill matches any required skill synonym
  if (requiredSynonyms.some(syn => syn.toLowerCase() === user)) {
    return true;
  }
  ...
  const userCleaned = user.replace(/[.\-_\s]/g, '');
  const requiredCleaned = required.replace(/[.\-_\s]/g, '');
  
  if (userCleaned === requiredCleaned) {
    return true;
  }
...
/**
 * Deduplicate an array of skills by normalizing them
 * This ensures that synonyms like "JavaScript", "JS", "js" all become "JavaScript"
 */
export function deduplicateSkills(skills) {
  ...
  skills.forEach(skill => {
    ...
    // Use the normalized form as the key to group synonyms
    if (!normalizedMap.has(normalizedLower)) {
      normalizedMap.set(normalizedLower, normalized);
    }
  });
